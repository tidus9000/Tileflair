...
public class MouseInputBuild : MonoBehaviour
{
    ...
    Ray GenerateTouchRay()
    {
        if (Input.touchCount > 0)
        {
            ...
            Vector3 touchPosFar = new Vector3(t.position.x,
                                                t.position.y,
                                                Camera.main.farClipPlane);
            Vector3 touchPosNear = new Vector3(t.position.x,
                                                t.position.y,
                                                Camera.main.nearClipPlane);
        ...
        switch (m_manager.GetState())
        {
                ...
                if (target)
                {
                    if (Input.touchCount > 0)
                    {
                        ...
                        if (t.phase == TouchPhase.Began)
                        {
                            ...
                            //newposition.y = m_floor.transform.position.y;
                            //m_activeObject = GameObject.Instantiate(m_selectedRoomObject, newposition, Quaternion.identity);
                            if (Physics.Raycast(touchRay.origin, touchRay.direction, out m_hit))
                            {
                                GameObject hitObj = m_hit.transform.gameObject;
                                Plane objPlane = new Plane(Vector3.up, hitObj.transform.position);
                                //Calculate offset
                        ...
                        if (t.phase == TouchPhase.Moved)
                        {
                            ...
                            Plane objPlane = new Plane(Vector3.up, hitObj.transform.position);
                            //Calculate offset
                            Ray mRay = Camera.main.ScreenPointToRay(t.position);
                            float rayDistance;
                            objPlane.Raycast(mRay, out rayDistance);
                            m_activeObject.transform.position = mRay.GetPoint(rayDistance) - hitObj.transform.position;
                        }
            ...
            case GameManager.State.VIEW:
                ...
                if (target)
                {
                    ...
                    if (Input.touchCount > 0)
                    {
                        velocityX += (xSpeed * Input.GetTouch(0).deltaPosition.x * distance * 0.002f) * Time.deltaTime;
                        velocityY += (ySpeed * Input.GetTouch(0).deltaPosition.y * 0.002f) * Time.deltaTime;
                    }
                    rotationYAxis += velocityX;
                    rotationXAxis -= velocityY;
                    rotationXAxis = ClampAngle(rotationXAxis, yMinLimit, yMaxLimit);
